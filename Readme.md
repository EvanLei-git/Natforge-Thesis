...
- **The Administrator:** Oversees the network, monitors active tunnels and users, manages the **regions (nodes)** (rename/enable/disable/remove self-registered nodes), and handles abuse via platform-wide **geo-blocking** by country.
...
- **Abuse Prevention:** The control plane supports **geo-blocking** by country, platform-wide (also gating login/registration) and per-tunnel, dropping connections whose source IP geolocates to a blocked country.
