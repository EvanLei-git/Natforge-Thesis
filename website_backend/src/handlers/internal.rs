...
pub async fn tunnel_up(
    ...
    queries::set_tunnel_online(&state.db.pg, p.tunnel_id, &p.node_id, p.agent_ip.as_deref())
        .await
        .map_err(err)?;
    tracing::info!("tunnel {} online on {} (agent {:?})", p.tunnel_id, p.node_id, p.agent_ip);
...
pub async fn policy(
    ...
    let per_tunnel = queries::all_tunnel_region_blocks(&state.db.pg).await.map_err(err)?;
    // Stringify the i64 keys so the map round-trips cleanly as JSON.
    let per_tunnel: std::collections::HashMap<String, Vec<String>> =
        per_tunnel.into_iter().map(|(k, v)| (k.to_string(), v)).collect();
    Ok(Json(json!({
        "blocked_ports": ports,
...
pub async fn node_register(
    ...
    .await
    .map_err(err)?;
    tracing::info!("node {} ({}) registered: {}", p.node_id, p.name, p.public_host);
    Ok(Json(json!({ "status": "ok" })))
}
