...
## Persistence
- **PostgreSQL** via `sqlx` **runtime** queries (no compile-time `query!` macros → build needs no DB). Migrations `0001..0007` applied at boot. Tables: `users, tunnels, routes, nodes, port_pool, bandwidth_logs, connection_logs, region_blocks, tunnel_region_blocks, reserved_subdomains`.
